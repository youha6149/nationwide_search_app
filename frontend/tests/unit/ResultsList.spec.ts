...
describe('ResultsList', () => {
    ...
    it('renders each address card with correct content', () => {
        ...
        addressCards.forEach((card, index) => {
            ...
            expect(card.find('h3').text()).toBe(`${address.prefecture} ${address.city}`)
            const paragraphs = card.findAll('p')
            expect(paragraphs[0].text()).toBe(`${address.postal_code} ${address.town} ${address.chome}`.trim())
            expect(paragraphs[1].text()).toBe(`${address.business_name} ${address.business_address}`.trim())
        })
    })
})
